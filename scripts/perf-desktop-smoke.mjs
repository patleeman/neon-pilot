...
async function main() {
  ...
  try {
    ...
    const interaction = await evalJs(
      cdp,
      `(async()=>{ const t=[]; function m(n,f){const s=performance.now(); f(); t.push([n, performance.now()-s]);} m('commandPalette',()=>window.dispatchEvent(new KeyboardEvent('keydown',{key:'k',metaKey:true,bubbles:true}))); await new Promise(r=>requestAnimationFrame(r)); const el=document.querySelector('textarea,[contenteditable="true"],input'); if(el){m('composerFocus',()=>el.focus()); m('type100',()=>{ if(el instanceof HTMLTextAreaElement || el instanceof HTMLInputElement){ const setter=Object.getOwnPropertyDescriptor(Object.getPrototypeOf(el),'value')?.set; setter?.call(el,'x'.repeat(100)); el.dispatchEvent(new InputEvent('input',{bubbles:true,inputType:'insertText',data:'x'.repeat(100)}));} else {el.textContent='x'.repeat(100); el.dispatchEvent(new InputEvent('input',{bubbles:true,inputType:'insertText',data:'x'.repeat(100)}));}}); await new Promise(r=>requestAnimationFrame(r)); m('type500',()=>{ if(el instanceof HTMLTextAreaElement || el instanceof HTMLInputElement){ const setter=Object.getOwnPropertyDescriptor(Object.getPrototypeOf(el),'value')?.set; setter?.call(el,'y'.repeat(500)); el.dispatchEvent(new InputEvent('input',{bubbles:true,inputType:'insertText',data:'y'.repeat(500)}));} else {el.textContent='y'.repeat(500); el.dispatchEvent(new InputEvent('input',{bubbles:true,inputType:'insertText',data:'y'.repeat(500)}));}})} return t;})()`,
    );
