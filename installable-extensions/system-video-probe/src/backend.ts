...
async function readHfToken(ctx: ExtensionBackendContext): Promise<string> {
  return (await ctx.secrets.get(HF_TOKEN_SECRET_ID))?.trim() || readLegacyHfToken();
}

async function readToolHfToken(ctx: unknown): Promise<string> {
  const maybeCtx = ctx as { secrets?: { get?: (secretId: string) => Promise<string | undefined> } } | undefined;
  return (await maybeCtx?.secrets?.get?.(HF_TOKEN_SECRET_ID))?.trim() || readLegacyHfToken();
}
  ...
  return (pi: ExtensionAPI) => {
    pi.registerTool({
      ...
      async execute(_toolCallId, params, signal, _onUpdate, ctx) {
        ...
        const settings = loadSettings();
        const hfToken = await readToolHfToken(ctx);
