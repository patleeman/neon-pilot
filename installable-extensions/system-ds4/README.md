...
The extension settings include an advanced config section for context window, max response tokens, KV disk cache size, and DS4 context intervention toggles. Context and KV cache changes are applied to the managed `ds4-server` launch after restart; model metadata and prompt/session intervention settings are refreshed when settings are saved.
