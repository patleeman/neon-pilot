...
When that model is selected, the extension keeps the live tool set to `bash`, `read`, `edit`, and `subagent`. Typical non-core tool affordances are intentionally offloaded to the `ds4` CLI that the extension adds to DS4 bash sessions, keeping the prompt and tool schema surface small and prompt-cache stable.
