...
The extension settings include an advanced config section for context window, max response tokens, and KV disk cache size. Context and KV cache changes are applied to the managed `ds4-server` launch after restart; model metadata is refreshed when settings are saved.
