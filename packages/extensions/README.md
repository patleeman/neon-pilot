...
Repeated backend infrastructure failures trip a per-extension circuit breaker. Three failures in a rolling ten-minute window disables the extension and shows a diagnostic in Extension Manager. This covers backend load/import failures, health checks, service startup, and similar host-level failures; normal action handler errors are returned to the caller and do not quarantine the extension. On clean startup, the runtime clears its extension startup marker after backend health checks, startup actions, services, and subscriptions are installed; if the previous launch left that marker behind, extension safe mode disables enabled runtime/user extensions before loading them again.
