...
- Surfaces: `SurfacePanel`, `PanelHeader`, `PanelMessage`, `CompactCard`, `ContextRail`, `ContextRailHeader`, `ContextRailBody`, `ContextRailSection`, `WorkbenchShell`, `WorkbenchHeader`, `RailSection`, `RailSubsection`, `ShelfSection`, `ShelfHeader`, `ShelfBody`
- Overlays: `Dialog`, `DialogHeader`, `DialogBody`, `DialogFooter`, `ConfirmDialog`, `TextPromptDialog`
- Feedback: `QuietLoadingState`, `CenteredState`, `CenteredLoadingState`, `CenteredMessage`, `LoadingState`, `ErrorState`, `EmptyState`
...
- Before building an app or extension route, choose one approved page type from `docs/design/page-template-standards.md`: Conversation, Table, Editor, Settings, Dashboard, or Setup. Set `nav[].pageType` on first-party route manifests, and document missing behavior before adding a new recurring page type.
- Treat side regions as design-system templates too. Route-owned contextual-left content starts with `SidebarSection` plus `SidebarList`/`SidebarRow`/`SidebarMessage`; route-owned right-sidebar content starts with `ContextRail` plus `ContextRailHeader`/`ContextRailBody`/`ContextRailSection`. See the `Patterns/App Pages/RouteShellRegions` Storybook story for the canonical left + main + right composition.
- For app and first-party extension actions, follow `docs/design/action-button-standards.md`. The default action grammar is icon-first: common verbs use `IconButton`/`IconLink` with `aria-label` and `title`; ambiguous, primary, destructive, or domain-specific actions use icon plus text in `Button`/`ButtonLink`; inline detail actions use `TextButton`; transcript actions use `MessageActionButton`; composer actions use `ComposerActionButton`.
...
- Use `ContextRail`, `ContextRailHeader`, `ContextRailBody`, and `ContextRailSection` for route-owned right sidebars and tab-local right rails. Do not build local rail roots, headers, scroll containers, or section dividers; the shell plus these primitives own the right-region background, fixed header band, padding, scrolling, typography, and section rhythm.
...
- Use `StatusDot` for compact colored status markers, `RingStatusDot` for compact percent/budget/quota markers, and `Spinner` for compact inline progress markers. Use `QuietLoadingState` for route, shell-region, and lazy `Suspense` fallbacks that are only waiting on code or registry state. Use `LoadingState`, `Notice`, `ErrorState`, and `EmptyState` for larger feedback surfaces attached to a specific table, list, editor, setting, tool output, or object. Feature-page empty states should use `EmptyState` or `AppPageEmptyState` with an optional eyebrow, one operational body sentence, two or three `steps`, and one attached action. Avoid bare `No items yet` copy. Avoid page-specific loading/error markup unless the layout requires it. Page-level loading should not show decorative progress icons; reserve `Spinner` for local button, row, tool-output, and object-attached progress.
- Use `CenteredLoadingState` only for bounded work surfaces where visible loading copy is useful, not for generic route or shell-region fallbacks. Use `CenteredState` when the centered content is not a loading state.
...
- Use `AppPageLayout`, `AppPageIntro`, and `AppPageSection` for extension-owned routes, and follow `docs/design/page-template-standards.md` before inventing local page chrome. `AppPageIntro` should be title-only by default. Put search, filters, tabs, result counts, refresh, create/install actions, and pagination in `DataTableToolbar` and `DataTablePagination` for table/list pages.
...
- Use `ProgressBar`, `ProgressRow`, `StatGrid`, `Stat`, `KeyValueList`, `KeyValueTable`, `DataTable`, `ResourceListItem`, `CodeBlock`, and `Disclosure` for data display before composing raw rows or panels. `ResourceListItem` is for selectable resources and chooser rows; use its `leading` slot for compact file/folder icons. `ProgressRow` is for label/bar/value dashboard rows; `KeyValueList` is for vertical definition rows with optional actions; `KeyValueTable` is for compact two-to-four column summaries. Use `DataTableToolbar` for table tabs, summaries, filters, search, and actions. Use `DataTable` with `DataTableHead`, `DataTableBody`, `DataTableRow`, `DataTableHeaderCell`, and `DataTableCell` for structured tables; pass `columns={<colgroup>...</colgroup>}` and `tableClassName="table-fixed"` when fixed column sizing matters. Use `DataTableEmptyRow` for loading/no-results rows instead of hand-written `tr`/`td` placeholders, `DataTableActionGroup` for right-aligned row action clusters, and `DataTablePagination` for paged results. `Stat` accepts `valueClassName`, `detailClassName`, `labelPosition`, and children for status dots or other small overlays.

## Proven Replacement Targets

These production areas already use the shared package and are useful examples for agents:

- Extension page shell and states: `system-telemetry`, `system-extension-manager`
