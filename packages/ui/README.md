...
- Actions: `Button`, `ButtonLink`, `TextLink`, `ToolbarButton`, `TextButton`, `TitleButton`, `SidebarNavButton`, `SidebarRow`, `MessageActionButton`, `MediaPreviewButton`, `ComposerActionButton`, `EditorToolbar`, `EditorToolbarGroup`, `EditorToolbarButton`, `IconButton`, `IconLink`, `BrowsePathButton`, `CheckButton`, `TaskListItem`, `ActionTile`
...
- Use `SidebarSection`, `SidebarRow`, and `SidebarMessage` for extension views rendered in the host left sidebar. `SidebarSection` owns the uppercase accent section title and compact header actions; `SidebarRow` owns native thread-like row spacing, selected state, truncation, and optional leading/trailing slots; `SidebarMessage` owns empty/loading/sidebar note text. Prefer title-only `SidebarRow` items over local `title + description` rows unless the secondary text is state or metadata.
