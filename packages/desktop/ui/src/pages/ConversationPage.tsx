...
export function ConversationPage({ draft = false, conversationId }: { draft?: boolean; conversationId?: string | null }) {
  ...
  function addVideoAttachments(nextVideoAttachments: ComposerVideoAttachment[]) {
  ...
  async function addComposerFiles(files: File[]) {
    ...
    if (nextVideoAttachments.length > 0) {
  ...
  function removeVideoAttachment(i: number) {
      ...
      <div className="relative flex-1 min-h-0">
        ...
        >
          ...
          {showBlockingConversationLoadingState ? (
            ...
            <div className="ui-conversation-setup-empty ui-app-page-empty-shell ui-app-page-empty-shell-start px-4 sm:px-6">
              <div className={`${DRAFT_EMPTY_STATE_CONTENT_WIDTH_CLASS} text-left`}>{newConversationSetupAction}</div>
            </div>
          ) : (
            <AppPageEmptyState
              icon={
              ...
              title={isLiveSession ? 'No messages yet' : 'This conversation is empty'}
              body={
                isLiveSession
