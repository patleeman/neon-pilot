...
function BackgroundBashInlineOutput({
  ...
  return (
    <div
      className="ui-tool-block__output ui-tool-block__output--background border-t border-border-subtle/70 bg-black/10 px-2.5 py-2 max-h-96 overflow-y-auto"
      tabIndex={-1}
      {...transcriptTargetAttributes({ kind: 'background_run', runId })}
    >
      <span className="sr-only">input</span>
      <pre className="ui-tool-block__pre whitespace-pre-wrap break-words text-[11px] leading-relaxed opacity-80">
...
function RoutineActivityRows({ runs }: { runs: ConversationRoutineActivityRun[] }) {
  ...
  return (
    <>
      {runs.map((run) => (
        <div key={run.id} className="ui-tool-block__routine bg-black/5 px-2.5 py-2 font-sans text-[12px]">
  ...
  return (
    ...
    >
      ...
      {displayedLinkedRuns.length > 0 && !pinnedTool && !backgroundShellStart && (
        <div className="ui-tool-block__linked-runs border-t border-border-subtle/70 bg-black/5 px-2.5 py-2 text-[11px] font-sans">
          <SectionLabel tone="muted" className="mb-1.5 block opacity-70">
            {displayedLinkedRuns.length === 1 ? 'listed execution' : 'listed executions'}
          </SectionLabel>
          {hiddenRunCount > 0 && (
            <div className="ui-tool-block__linked-runs-summary mb-2 flex flex-wrap items-center gap-2 rounded-md bg-black/5 px-2 py-1.5 text-[10px] text-secondary/80">
              <span>
                {showAllRuns
          ...
          <div className="space-y-1.5">
            {visibleRuns.map((linkedRun) => {
              ...
              return (
                <div key={linkedRun.runId} className="ui-tool-block__linked-run w-full rounded-md px-2 py-1.5 text-left text-dim">
      ...
      {mentionedInlineRuns.length > 0 && !pinnedTool && !backgroundShellStart && (
        <div className="ui-tool-block__mentioned-runs space-y-1.5 border-t border-border-subtle/70 bg-black/5 px-2.5 py-2 font-sans">
      ...
      {open && !pinnedTool && agentBashTool && !bareLinkedRunOnly && (
        <div className="ui-tool-block__output ui-tool-block__output--bash border-t border-border-subtle/70 bg-black/10 px-2.5 py-2 max-h-96 overflow-y-auto">
          <span className="sr-only">input</span>
          <pre className="ui-tool-block__pre whitespace-pre-wrap break-words text-[11px] leading-relaxed opacity-80">
            <span className="opacity-60">$ </span>
            {bashCommand}
      ...
      {open && !pinnedTool && !agentBashTool && !backgroundShellStart && (
        <div className="ui-tool-block__body border-t border-border-subtle/70">
          {(isRunning || output || outputDeferred) && (
            <div className="ui-tool-block__output px-2.5 py-2 max-h-96 overflow-y-auto">
              {output ? (
                <pre className="ui-tool-block__pre whitespace-pre-wrap break-all text-[11px] leading-relaxed opacity-75">{output}</pre>
              ) : isRunning ? (
