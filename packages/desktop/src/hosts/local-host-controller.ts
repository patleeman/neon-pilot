...
export class LocalHostController implements HostController {
  ...
  async readScheduledTaskSchedulerHealth(): Promise<unknown> {
    const module = await this.loadLocalApi();
    return module.readDesktopScheduledTaskSchedulerHealth();
  }
  ...
  async resumeLiveSession(input: { sessionFile: string; cwd?: string }): Promise<{ id: string }> {
    const module = await this.loadLocalApi();
    return module.resumeDesktopLiveSession(input);
  }
