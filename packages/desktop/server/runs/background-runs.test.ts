...
describe('background runs', () => {
  ...
  it('stores shell command runs and trims argv-based shell targets', async () => {
    ...
    expect(shellManifest?.spec.metadata).toEqual(
  ...
  it('marks a run started and preserves checkpoint payload', async () => {
    ...
    expect(loadDurableRunCheckpoint(record.paths.checkpointPath)).toEqual(
      expect.objectContaining({
        step: 'spawned',
    ...
    expect(readDurableRunEvents(record.paths.eventsPath).at(-1)).toEqual(
      expect.objectContaining({
  ...
  it('finalizes failed and cancelled runs with status, events, and result files', async () => {
    ...
    expect(loadDurableRunStatus(failedRecord.paths.statusPath)).toEqual(
      expect.objectContaining({
        status: 'failed',
    ...
    expect(loadDurableRunCheckpoint(failedRecord.paths.checkpointPath)).toEqual(
      expect.objectContaining({
        step: 'failed',
        payload: expect.objectContaining({
          exitCode: 2,
    ...
    expect(JSON.parse(readFileSync(failedRecord.paths.resultPath, 'utf-8'))).toEqual(
      expect.objectContaining({
        runId: failedRecord.runId,
        exitCode: 2,
        signal: 'SIGTERM',
    ...
    expect(loadDurableRunStatus(cancelledRecord.paths.statusPath)).toEqual(
      expect.objectContaining({
        status: 'cancelled',
    ...
    expect(JSON.parse(readFileSync(cancelledRecord.paths.resultPath, 'utf-8'))).toEqual(
      expect.objectContaining({
        cancelled: true,
        success: false,
      }),
