...
import {
  ...
  type CompanionConversationBranchInput,
...
function readOptionalPositiveInteger(input: unknown, field: string): number | undefined {
  ...
  const value = typeof input === 'string' ? Number.parseInt(input, 10) : typeof input === 'number' ? input : Number.NaN;
...
function buildSetupState(
  ...
  const effectiveConfig: DaemonConfig = {
    ...
    companion: {
      ...
      port: portOverride ?? config.companion?.port ?? resolveNeonPilotRuntimeChannelConfig().companionPort,
    },
  };
...
export class DaemonCompanionServer {
  ...
  async start(): Promise<void> {
    ...
    const preferredPort = this.config.companion?.port ?? resolveNeonPilotRuntimeChannelConfig().companionPort;
