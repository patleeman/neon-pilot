  ...
  it('supports strongly typed extension manifests', () => {
    const manifest: ExtensionManifest = {
      ...
      contributes: {
        ...
        webapps: [{ id: 'board', title: 'Board Webapp', entry: 'dist/webapp/index.html', localhostName: 'board-agent-board' }],
        nav: [{ id: 'nav', label: 'Agent Board', route: '/ext/agent-board', icon: 'kanban', pageType: 'table' }],
        composerInputTools: [{ id: 'draw', component: 'DrawButton', title: 'Draw', when: '!streamIsStreaming' }],
        selectionActions: [
          { id: 'send-selection', title: 'Send Selection', action: 'sendSelection', kinds: ['text', 'messages'] },
          { id: 'inspect-resource', title: 'Inspect Resource', action: 'inspectResource', kinds: ['resource'] },
        ],
    ...
    expect(manifest.contributes?.webapps?.[0]?.localhostName).toBe('board-agent-board');
