...
export function listExtensionCliCommandRegistrations(): ExtensionCliCommandRegistration[] {
  return readExtensionRegistrySnapshot().extensions.flatMap((extension) =>
    (extension.contributes?.cliCommands ?? []).map((command) => ({
      ...
      ...(command.argsSchema ? { argsSchema: command.argsSchema } : {}),
      ...(command.flagsSchema ? { flagsSchema: command.flagsSchema } : {}),
      ...(command.mode ? { mode: command.mode } : {}),
      ...(typeof command.requiresApp === 'boolean' ? { requiresApp: command.requiresApp } : {}),
      ...(typeof command.destructive === 'boolean' ? { destructive: command.destructive } : {}),
      ...(typeof command.idempotent === 'boolean' ? { idempotent: command.idempotent } : {}),
      ...(typeof command.startsBackgroundWork === 'boolean' ? { startsBackgroundWork: command.startsBackgroundWork } : {}),
      ...(typeof command.supportsDryRun === 'boolean' ? { supportsDryRun: command.supportsDryRun } : {}),
      ...(command.outputModes?.length ? { outputModes: command.outputModes } : {}),
      ...(command.streaming ? { streaming: command.streaming } : {}),
      ...(command.smoke ? { smoke: command.smoke } : {}),
