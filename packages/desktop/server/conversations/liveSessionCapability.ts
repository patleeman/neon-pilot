...
function buildBackgroundRunInternalContext(entries: Array<{ prompt: string }>): string {
  ...
  const lines = [
    ...
    'If the only sensible next step is to wait and inspect again later, add a conversation action `deferred_resume` item with trigger "delay" or "at" yourself instead of asking the user to remind you.',
  ];
