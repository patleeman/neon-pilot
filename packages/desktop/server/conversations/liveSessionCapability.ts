...
function buildBackgroundRunInternalContext(entries: Array<{ prompt: string }>): string {
  ...
  const lines = [
    ...
    'If the only sensible next step is to wait and inspect again later, use the canonical neon_pilot admin surface to schedule a delayed conversation follow-up instead of asking the user to remind you.',
  ];
