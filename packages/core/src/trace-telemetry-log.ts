...
function resolveMaxLogFileBytes(): number {
  const raw = process.env.NEON_PILOT_TRACE_TELEMETRY_LOG_MAX_BYTES ?? process.env.NEON_PILOT_APP_TELEMETRY_LOG_MAX_BYTES;
...
function resolveRetentionDays(): number {
  const raw = process.env.NEON_PILOT_TRACE_TELEMETRY_LOG_RETENTION_DAYS ?? process.env.NEON_PILOT_APP_TELEMETRY_LOG_RETENTION_DAYS;
