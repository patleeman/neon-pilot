...
function getConversationCheckpointStateRoot(stateRoot?: string): string {
...
export function validateConversationCheckpointId(checkpointId: string): void {
  if (!CHECKPOINT_ID_PATTERN.test(checkpointId)) {
    throw new Error(
      `Invalid checkpoint id "${checkpointId}". Checkpoint ids may only include letters, numbers, dots, dashes, and underscores.`,
    );
  }
}
...
function normalizeCheckpointSource(source: ConversationCheckpointSource): ConversationCheckpointSource {
...
function normalizeCheckpointAnchor(anchor: ConversationCheckpointAnchor): ConversationCheckpointAnchor {
...
function countSnapshotLines(content: string): number {
...
function normalizeCheckpointSnapshot(value: unknown): ConversationCheckpointSnapshot {
  if (!value || typeof value !== 'object') {
    throw new Error('Checkpoint snapshot metadata is required.');
  }

  const snapshot = value as Partial<ConversationCheckpointSnapshot>;
...
function normalizeCheckpointRecord(value: unknown): ConversationCheckpointRecord {
  if (!value || typeof value !== 'object') {
    throw new Error('Checkpoint document is invalid.');
  }

  const parsed = value as Partial<ConversationCheckpointRecord>;
  ...
  return {
    ...
    source: normalizeCheckpointSource(parsed.source as ConversationCheckpointSource),
    anchor: normalizeCheckpointAnchor(parsed.anchor as ConversationCheckpointAnchor),
    snapshot: normalizeCheckpointSnapshot(parsed.snapshot),
  };
}

function readCheckpointRecordFromPath(path: string): ConversationCheckpointRecord {
...
export function getConversationCheckpoint(options: ResolveConversationCheckpointPathOptions): ConversationCheckpointRecord | null {
...
export function saveConversationCheckpoint(options: SaveConversationCheckpointOptions): ConversationCheckpointRecord {
  validateProfileName(options.profile);
  ...
  const record: ConversationCheckpointRecord = {
    version: CHECKPOINT_VERSION,
    id: checkpointId,
    title,
    ...(note ? { note } : {}),
...
export function deleteConversationCheckpoint(options: ResolveConversationCheckpointPathOptions): boolean {
  validateProfileName(options.profile);
  validateConversationCheckpointId(options.checkpointId);
