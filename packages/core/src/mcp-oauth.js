...
function buildProtectedResourceMetadataUrls(serverUrl) {
  const url = new URL(serverUrl);
...
async function discoverProtectedResourceMetadata(serverUrl, wwwAuthenticateHeader) {
...
function handleWaitForAuth(req, res, state) {
...
function setupOAuthCallbackServer(options) {
  let authCode = null;
  let authCompletedResolve = null;
  let authCompletedReject = null;
  const authCompletedPromise = new Promise((resolve, reject) => {
...
async function connectToRemoteServer(options) {
  const recursionReasons = options.recursionReasons ?? new Set();
...
class PersonalAgentOAuthClientProvider {
  input;
  stateId = randomUUID();
  cachedClientInfo;
  constructor(input) {
  ...
  async clientInformation() {
    ...
    const clientInfo = await readJsonFile(this.input.serverUrlHash, 'client_info.json', OAuthClientInformationFullSchema);
  ...
  async tokens() {
    return readJsonFile(this.input.serverUrlHash, 'tokens.json', OAuthTokensSchema);
  }
  ...
  async discoveryState() {
    return readJsonFile(this.input.serverUrlHash, 'discovery.json', {
...
export function getMcpServerUrlHash(serverUrl, authorizeResource, headers) {
...
export async function resolveCallbackPort(serverUrlHash, preferredPort) {
...
export async function openRemoteMcpClient(options) {
