...
6. App-first runtime cutover.
   ...
   - 2026-07-05 progress: server route registry and App Manager catalog copy now treat `/apps` as canonical, preserve `/extensions` as a legacy alias, label the manager as App Manager, and present first-party/configured sources as app repositories/app packages while leaving the extension package type as the runtime boundary.
   - 2026-07-05 validation: App Manager frontend/manifest tests plus desktop server catalog, manifest, registry, and route smoke tests pass for the app-facing route/copy cleanup.
   - 2026-07-05 progress: stale compatibility diagnostics now say app package instead of Extension, and App Manager test/copy fallbacks now use app/app-package wording for route descriptions, open-folder failures, and install catalog fixtures.
   - 2026-07-05 validation: App Manager frontend plus server compatibility/catalog/registry tests pass for the compatibility-copy cleanup.
   ...
   - 2026-07-05 progress: Browser, Files, and Terminal are now canonical Windowed OS app registrations with app-package owners, Start menu aliases, default sizes, and standalone launcher behavior. Chat-attached child windows keep their existing parent lifecycle, while Start menu launches create standalone `app:browser`, `app:files`, and `app:terminal` windows using the same native app surfaces.
