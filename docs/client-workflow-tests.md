...
| Chat primary | Existing conversation opens from sidebar, updates active id, and survives reload hydration | `ui/src/components/Sidebar.nested.test.tsx` covers rendered remote layout hydration for an existing active conversation and verifies visible row, active state, and local persistence. Add a full App route test for navigation from the row. |
