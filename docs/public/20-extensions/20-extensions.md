...
Each command should include a human-readable `description`; add `usage` and `examples` when the command accepts arguments or flags. System extension CLI commands should be covered by command discovery and help-output checks.
...
The backend API is deliberately two-layered: public SDK stubs under `@neon-pilot/extensions/backend/*`, and host implementations inside the desktop runtime. Extension source imports only `@neon-pilot/extensions` and `@neon-pilot/extensions/backend/{name}`. It must not import desktop server files, `@neon-pilot/core`, `@neon-pilot/daemon`, or agent-runtime internals directly. System extension source should use the public `ExtensionAPI` type from `@neon-pilot/extensions` for agent lifecycle hooks, and any missing host capability should become a focused SDK/backend seam. Host backend APIs should stay thin, lazy-load heavy host/runtime code, and use the worker capability bridge when a capability may run in an extension backend worker.
...
2. For frontend, route, nav, sidebar, manifest, or shared UI changes, run the desktop UI build so the renderer sees the current source and contributions.
