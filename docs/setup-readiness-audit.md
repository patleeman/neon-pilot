...
- Settings-managed capabilities such as MCP servers and model providers already have dedicated configuration/status surfaces. They should add setup readiness only when an enabled extension has a concrete missing prerequisite that makes its contributed tools/controls unusable.
