...
Worker-backed action coverage now includes product-critical system surfaces for artifacts, Caffeinate process control, code mode, Codex apply-patch, conversation context-menu helpers, the worker-safe subset of the conversation tool (`create`, `set_title`, `ensure_live`, `send_message`, `abort`, `compact`, `fork`, `set_active_tools`, `workspace_get`, `workspace_update`, `append_transcript_block`, `update_transcript_block`, and `rollback`), extension manager read/no-op actions, knowledge state/read/search/reference/memory actions and non-streaming read routes, local dictation settings/model install/transcription, MCP settings/read-only tool inspection, onboarding bootstrap, prompt assembly inspection/toggles, read-only skills inventory, telemetry aggregate reads, todos, web fetch, installable web search tools, installable local model lookup actions, installable DS4 provider/tool actions, Video Probe local runtime controls, Suggested Context cache warming, and background work (`system-runs`). Background work can cross the worker boundary for non-streaming action paths because it uses public backend subpaths, serialized tool context metadata, non-streaming shell execution, and `ctx.ui.invalidate`; Caffeinate can cross the boundary because `ctx.shell.spawn` now returns worker-side handles for host-owned process lifecycle methods and can deliver stdout/stderr/exit callbacks back to the worker. Live foreground streaming still stays on the host runner when a tool update callback or live agent abort context is present.
