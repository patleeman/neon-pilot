...
Worker-backed action coverage now includes product-critical system surfaces for artifacts, code mode, Codex apply-patch, conversation context-menu helpers, the worker-safe subset of the conversation tool (`create`, `set_active_tools`, and `append_transcript_block`), local dictation settings, MCP settings/read-only tool inspection, onboarding bootstrap, prompt assembly inspection, telemetry aggregate reads, todos, web fetch, and background work (`system-runs`). Background work can cross the worker boundary for non-streaming action paths because it uses public backend subpaths, serialized tool context metadata, non-streaming shell execution, and `ctx.ui.invalidate`; live foreground streaming still stays on the host runner when a tool update callback or live agent abort context is present.
