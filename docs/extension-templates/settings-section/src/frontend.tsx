...
export function MySettingsSection({ pa }: { pa: NativeExtensionClient }) {
  ...
  useEffect(() => {
    pa.actions
      ...
      .catch((err: Error) => {
        pa.ui.notify({ type: 'error', message: `Failed to load settings: ${err.message}`, source: 'my-settings-section' });
      })
  ...
  const save = useCallback(
    async (event: React.FormEvent) => {
      ...
      setNotice(null);
      try {
        await pa.actions.call('mySettingsSave', settings);
        setNotice('Settings saved.');
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        pa.ui.notify({ type: 'error', message: `Failed to save settings: ${msg}`, source: 'my-settings-section' });
      } finally {
        setSaving(false);
      }
    },
