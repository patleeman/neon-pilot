...
Neon Pilot stores the managed mirror under the runtime state directory and syncs it in the background. Git operations are bounded so a stuck credential prompt or unreachable remote records an error instead of leaving the Knowledge page permanently loading.
