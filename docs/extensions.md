...
The extension backend worker entrypoint currently supports wire-safe backend import/cache invalidation, export-availability checks, and narrow export execution with worker-safe context handles. The worker import runner uses a separate backend worker per extension for those operations. The worker transport also supports worker-to-host capability requests: a worker sends a typed capability name, operation name, and serializable input, and the host returns a typed success or error response through the same worker channel. The first worker-safe backend context handles are serialized `ctx.runtime` metadata, `ctx.log`, `ctx.events.publish`, narrow conversation handles (`ctx.conversations.get`, `create`, `setActiveTools`, `appendCustomEntry`, `appendTranscriptBlock`, and `metadata`), extension registry reads/enablement through `ctx.extensions`, `ctx.git`, read-only `ctx.models.list`, `ctx.notify`, `ctx.storage`, `ctx.secrets.get`, non-streaming `ctx.shell.exec`, `ctx.telemetry.record`, `ctx.ui.invalidate`, and the serialized `ctx.workspace` file API. Product-critical system surfaces including artifacts, code mode, Codex apply-patch, conversation context-menu helpers, the worker-safe subset of the conversation tool (`create`, `set_active_tools`, and `append_transcript_block`), local dictation settings, MCP settings/read-only tool inspection, onboarding bootstrap, prompt assembly inspection, read-only skills inventory, telemetry aggregate reads, todos, web fetch, and background work (`system-runs`) run their declared worker-safe actions through this lane; broader backend handler execution still runs through the in-process runner until the remaining live capabilities are implemented on top of those capability handles.
