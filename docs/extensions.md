...
The extension backend worker entrypoint currently supports wire-safe backend import/cache invalidation and export-availability checks. The worker import runner uses a separate backend worker per extension for those operations. The worker transport also supports worker-to-host capability requests: a worker sends a typed capability name, operation name, and serializable input, and the host returns a typed success or error response through the same worker channel. Backend handler execution still runs through the in-process runner until host contexts and live capabilities are implemented on top of those capability handles.
