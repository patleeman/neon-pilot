...
describe('RoutinesPage', () => {
  ...
  it('renders a native windowed routines surface when hosted by the windowed shell', async () => {
    ...
    const dialog = screen.getByRole('dialog', { name: 'Review code changes' });
    expect(dialog).toBeTruthy();
    expect(container.querySelector('.wos-dialog-stack')).not.toBeNull();
    expect(container.querySelector('.wos-dialog-stack .wos-field')).not.toBeNull();
    expect(container.querySelector('.wos-dialog-stack .wos-select')).not.toBeNull();
    expect(container.querySelector('.wos-dialog-stack .wos-input')).not.toBeNull();
    expect(container.querySelector('.wos-dialog-stack .wos-textarea')).not.toBeNull();
    expect(dialog.querySelector('.wos-settings-group')).not.toBeNull();
    expect(dialog.querySelector('.wos-form-grid')).not.toBeNull();
    expect(dialog.querySelector('.ui-flat-panel')).toBeNull();
    expect(dialog.querySelector('.ui-callout-warning')).toBeNull();
    expect(dialog.querySelector('.border-t.border-border-subtle')).toBeNull();
