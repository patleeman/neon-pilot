  ...
  for (let attempt = 0; ; attempt++) {
    try {
      ...
      if (!res.ok && RETRYABLE_STATUS_CODES.includes(res.status) && attempt < RETRY_DELAYS_MS.length) {
        await sleep(RETRY_DELAYS_MS[attempt], init?.signal ?? void 0);
        continue;
      }
      return res;
    } catch (error) {
...
async function get(path, options) {
  return requestJson("GET", path, void 0, options);
}
...
var api = {
  ...
  sessionDetail: async (id, options) => {
    ...
    return get(`/sessions/${encodeURIComponent(id)}${query ? `?${query}` : ""}`, { signal: options?.signal });
  },
  sessionBlock: async (id, blockId, options) => {
    return get(`/sessions/${encodeURIComponent(id)}/blocks/${encodeURIComponent(blockId)}`, { signal: options?.signal });
  },
  sessionEntryBlocks: async (id, entryIds, options) => {
    return requestJson(
      "POST",
  ...
  createLiveSession: async (cwd, text, options) => {
    ...
    const result = await post("/live-sessions", {
      ...
      ...options?.audios !== void 0 ? { audios: options.audios } : {},
  ...
  promptSession: async (id, text, behavior, images, videos, audios, documents, attachmentRefs, surfaceId, contextMessages, relatedConversationIds) => {
    const startedAtMs = performance.now();
  ...
  sendConversationMessage: async (id, text, behavior, images, videos, audios, documents, attachmentRefs, surfaceId, contextMessages, relatedConversationIds) => {
    const startedAtMs = performance.now();
