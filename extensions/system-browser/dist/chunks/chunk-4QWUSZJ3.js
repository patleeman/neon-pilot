...
var EXTENSION_REGISTRY_CHANGED_EVENT = "neon-pilot-extension-registry-changed";
...
function normalizeTranscriptBlocks(extensions) {
  const result = [];
  for (const extension2 of extensions) {
    const blocks = extension2.contributes?.transcriptBlocks;
...
function prepareStreamingMarkdownText(text7) {
  ...
  return `${text7}${text7.endsWith("\n") ? "" : "\n"}${closingFence}`;
}
function renderStreamingMarkdownText(text7, options) {
  return renderMarkdownText(prepareStreamingMarkdownText(text7), options);
}
...
function lazyTranscriptBlockComponent(extension2, renderer, revision) {
  return lazy(async () => {
    const module = await loadExtensionModule(extension2, revision);
    const component = module[renderer.component];
...
function NativeExtensionTranscriptBlockHost({
  extension: extension2,
  renderer,
  block,
  context
  ...
  useExtensionStyles(extension2.id, extension2.manifest.frontend?.styles);
  const pa = useMemo(() => createNativeExtensionClient(extension2.id), [extension2.id]);
  const blockContext = useMemo(
    () => ({
      ...context,
      renderMarkdown: (markdown) => renderMarkdownText(markdown)
    }),
    [context]
  );
  const moduleKey = extensionModuleKey(extension2);
  const Component = useMemo(
    () => lazyTranscriptBlockComponent(extension2, renderer, getExtensionRegistryRevision()),
    [extension2, renderer, moduleKey]
  );
  return /* @__PURE__ */ jsx(Suspense, { fallback: /* @__PURE__ */ jsx(LoadingState, { label: "Loading transcript block\u2026", className: "py-3" }), children: /* @__PURE__ */ jsx(ExtensionToolBlockErrorBoundary, { extensionId: extension2.id, children: /* @__PURE__ */ jsx(Component, { pa, block, renderer, context: blockContext }) }) });
}
...
function readSpec(run, key) {
  return isRecord2(run.manifest?.spec) ? readString(run.manifest.spec, key) : void 0;
}
function readNestedSpec(run, key, nestedKey) {
  if (!isRecord2(run.manifest?.spec)) {
    return void 0;
  }
  const value = run.manifest.spec[key];
  return isRecord2(value) ? readString(value, nestedKey) : void 0;
}
function readCheckpoint(run, key) {
  return isRecord2(run.checkpoint?.payload) ? readString(run.checkpoint.payload, key) : void 0;
}
function readNestedCheckpoint(run, key, nestedKey) {
  if (!isRecord2(run.checkpoint?.payload)) {
    return void 0;
  }
...
// packages/desktop/ui/src/hooks/useDesktopConversationState.ts
var DESKTOP_CONVERSATION_STATE_REFRESH_EVENT = "neon-pilot:desktop-conversation-state-refresh";
function notifyDesktopConversationStateRefresh(conversationId) {
...
var iconButtonClassName = "ui-message-action-button-icon";
function MessageActionTooltip({ label, children }) {
  return /* @__PURE__ */ jsxs("span", { className: "ui-tooltip-host relative inline-flex", children: [
    children,
    /* @__PURE__ */ jsx(Tooltip, { position: "top-right", children: label })
  ] });
}
function extensionActionIcon(action) {
...
function extensionActionSource(action) {
...
function cleanExtensionActionErrorMessage(action, error) {
...
function readRefreshConversationId(result) {
  if (!result || typeof result !== "object" || Array.isArray(result)) {
    return "";
  }
  const value = result.refreshConversationId;
...
function MessageActions({
  ...
  const [actionErrors, setActionErrors] = useState(/* @__PURE__ */ new Map());
  const [actionStatuses, setActionStatuses] = useState(/* @__PURE__ */ new Map());
  const [copyState, setCopyState] = useState("idle");
  const copyResetTimeoutRef = useRef(null);
  const actionStatusResetTimeoutsRef = useRef(/* @__PURE__ */ new Map());
  ...
  return /* @__PURE__ */ jsxs(
    "div",
    {
      ...
      children: [
        canCopy && /* @__PURE__ */ jsx(MessageActionTooltip, { label: copyState === "failed" ? "Copy to clipboard failed" : copyTitle, children: /* @__PURE__ */ jsx(
          MessageActionButton,
          {
            type: "button",
            onClick: () => {
            ...
            children: copyState === "copied" ? "\u2713" : copyState === "failed" ? "!" : "\u2398"
          }
        ) }),
        onEdit && /* @__PURE__ */ jsx(MessageActionTooltip, { label: "Edit this prompt and rerun the conversation from here", children: /* @__PURE__ */ jsx(
          MessageActionButton,
          {
            type: "button",
            onClick: onEdit,
        ...
        onRewind && /* @__PURE__ */ jsx(
          ...
          {
            label: isUser ? "Rewind into a new conversation from this prompt" : "Rewind into a new conversation from the prompt that led here",
            children: /* @__PURE__ */ jsx(
              MessageActionButton,
              {
                type: "button",
                ...
                "aria-label": isUser ? "Rewind into a new conversation from this prompt" : "Rewind into a new conversation from the prompt that led here",
                disabled: isRewinding,
                className: iconButtonClassName,
                children: isRewinding ? "\u2026" : "\u21A9"
              }
            )
          }
        ),
        ...
        messageActions.map((action) => {
          ...
          const title = actionError ? `${action.title} failed. See notification.` : actionStatus ? `${action.title}: ${actionStatus}` : action.title;
          return /* @__PURE__ */ jsx(MessageActionTooltip, { label: title, children: /* @__PURE__ */ jsx(
            MessageActionButton,
            {
              type: "button",
              onClick: () => {
                void (async () => {
                  ...
                  try {
                    ...
                    const statusText = result && typeof result === "object" && "text" in result && typeof result.text === "string" ? result.text.trim() : "";
                    if (statusText) {
                      const existingTimeoutId = actionStatusResetTimeoutsRef.current.get(action.id);
                      if (existingTimeoutId !== void 0) window.clearTimeout(existingTimeoutId);
              ...
              disabled: busy,
              className: iconButtonClassName,
              children: actionError ? "!" : busy ? "\u2026" : extensionActionIcon(action)
            }
          ) }, action.id);
        })
      ]
...
var STANDALONE_CONTEXT_CUSTOM_TYPES = /* @__PURE__ */ new Set(["model_arena_duel"]);
...
function isStandaloneContextBlock(block) {
  return block.type === "context" && STANDALONE_CONTEXT_CUSTOM_TYPES.has(block.customType ?? "");
}
function readModelArenaDuelDetails(block) {
  if (block.type !== "context" || block.customType !== "model_arena_duel" || !isRecord6(block.details)) return null;
  const sourceBlockId = readString2(block.details.sourceBlockId);
  const sideA = isRecord6(block.details.sideA) ? block.details.sideA : {};
  const sideB = isRecord6(block.details.sideB) ? block.details.sideB : {};
  const models = isRecord6(block.details.models) ? block.details.models : null;
  return {
    sourceBlockId,
    status: readString2(block.details.status),
    sideA: { role: readString2(sideA.role), text: readString2(sideA.text) },
    sideB: { role: readString2(sideB.role), text: readString2(sideB.text) },
    models: models ? {
      a: readString2(models.a) || void 0,
      b: readString2(models.b) || void 0,
      primary: readString2(models.primary) || void 0,
      challenger: readString2(models.challenger) || void 0
    } : null,
    vote: typeof block.details.vote === "string" ? block.details.vote : null
  };
}
function buildModelArenaVariationSet(sourceBlock, duelBlock) {
  ...
  if (!sourceBlockId || details.sourceBlockId && !sourceBlockIdAliases(details.sourceBlockId).some((alias) => sourceBlockIdAliases(sourceBlockId).includes(alias))) {
    return null;
  }
  const challengerSide = details.sideA.role === "challenger" ? details.sideA : details.sideB.role === "challenger" ? details.sideB : details.sideA.text === sourceBlock.text ? details.sideB : details.sideB.text === sourceBlock.text ? details.sideA : null;
    ...
    variations: [
      {
        ...
        label: details.models?.primary ? `Current model \xB7 ${details.models.primary}` : "Current model",
        text: sourceBlock.text,
        modelRef: details.models?.primary
      },
      {
        id: `${duelBlock.id ?? sourceBlockId}:challenger`,
        label: details.models?.challenger ? `Challenger \xB7 ${details.models.challenger}` : "Challenger",
        text: challengerText,
...
function findLegacyModelArenaSourceBlockId(messages, duelIndex, details) {
  for (let index2 = duelIndex - 1; index2 >= 0; index2 -= 1) {
    const block = messages[index2];
    if (block?.type !== "text" || !block.id) continue;
    if (block.text === details.sideA.text || block.text === details.sideB.text) return block.id;
  }
  for (let index2 = duelIndex + 1; index2 < messages.length; index2 += 1) {
    const block = messages[index2];
    if (block?.type !== "text" || !block.id) continue;
    if (block.text === details.sideA.text || block.text === details.sideB.text) return block.id;
  }
...
function sourceBlockIdAliases(sourceBlockId) {
...
function canonicalArenaSourceBlockId(sourceBlockId) {
  return sourceBlockId.trim().replace(/-x\d+$/, "");
}
function arenaStatusIsActive(status) {
  return status !== "cancelled" && status !== "voted";
}
function modelArenaDuelHasBothAnswers(details) {
  return Boolean(details.sideA.text.trim() && details.sideB.text.trim());
}
function findAssistantBlockByArenaSourceId(assistantById, sourceBlockId) {
...
function addArenaSourceHiddenId(hiddenAssistantBlockIds, sourceBlockId) {
  for (const alias of sourceBlockIdAliases(sourceBlockId)) {
    hiddenAssistantBlockIds.add(alias);
  }
}
function deleteArenaSourceHiddenId(hiddenAssistantBlockIds, sourceBlockId) {
  for (const alias of sourceBlockIdAliases(sourceBlockId)) {
    hiddenAssistantBlockIds.delete(alias);
  }
}
function collectModelArenaPresentation(messages) {
  const assistantById = /* @__PURE__ */ new Map();
  for (const block of messages) {
    if (block.type === "text" && block.id) assistantById.set(block.id, block);
  }
  const hiddenAssistantBlockIds = /* @__PURE__ */ new Set();
  const hiddenDuelBlockIds = /* @__PURE__ */ new Set();
  const variationSetsBySourceBlockId = /* @__PURE__ */ new Map();
  const activeDuelBySourceBlockId = /* @__PURE__ */ new Map();
  const votedDuelBySourceBlockId = /* @__PURE__ */ new Map();
  for (const [index2, block] of messages.entries()) {
    if (block.type !== "context") continue;
    const details = readModelArenaDuelDetails(block);
    if (!details) continue;
    const sourceBlockId = details.sourceBlockId || findLegacyModelArenaSourceBlockId(messages, index2, details);
...
function shouldRebuildPreviousClusterForAppend(previousLastItem, nextBlock, standaloneTools) {
  ...
  return isTraceConversationBlock(nextBlock, standaloneTools) || isContextConversationBlock(nextBlock) && !isStandaloneContextBlock(nextBlock);
}
function modelArenaRebuildStartIndex(messages, changedIndex) {
...
function buildChatRenderItems(messages, standaloneTools = /* @__PURE__ */ new Set()) {
  ...
  for (const [index2, block] of messages.entries()) {
    ...
    if (isStandaloneContextBlock(block)) {
      flushTraceBlocks();
      flushContextBlocks();
      items.push({ type: "context_cluster", blocks: [block], startIndex: index2, endIndex: index2 });
      continue;
    }
    ...
    items.push({
      ...
      block,
      index: index2,
      arenaVariationSet: block.type === "text" && block.id ? arenaPresentation.variationSetsBySourceBlockId.get(block.id) : void 0
    });
  }
  flushTraceBlocks();
  flushContextBlocks();
...
function buildChatRenderItemsIncremental(input) {
  ...
  if (arenaRebuildStartIndex !== null) {
    return [
      ...
      ...buildChatRenderItems(input.messages.slice(arenaRebuildStartIndex), standaloneTools).map(
        (item) => shiftChatRenderItemIndex(item, arenaRebuildStartIndex)
      )
    ];
  }
...
function modelArenaTranscriptBlockKey(block, fallback) {
  const details = block.details && typeof block.details === "object" && !Array.isArray(block.details) ? block.details : {};
...
function readModelArenaDuelBlockData(details) {
  if (!isRecord7(details) || typeof details.duelId !== "string") return null;
...
function modelArenaSideText(side) {
  return side?.text?.trim() ?? "";
}
function modelArenaHasBothAnswers(duel) {
  return Boolean(modelArenaSideText(duel.sideA) && modelArenaSideText(duel.sideB));
}
function modelArenaPreferredChallengerModel(duel, choice) {
...
function mergeModelArenaSide(current, incoming) {
...
function mergeModelArenaDuelBlockData(current, incoming) {
...
function ModelArenaDuelContextBlock({ block }) {
  ...
  useEffect(() => {
    if (!local || local.status !== "running" || modelArenaHasBothAnswers(local)) return void 0;
    let cancelled = false;
    const refresh = async () => {
      try {
        const result = await client.extension.invoke("refreshDuel", { duelId: local.duelId });
  ...
  const vote = async (choice) => {
    ...
    setBusy(choice);
    try {
      const result = await client.extension.invoke("voteDuel", { duelId: local.duelId, choice });
  ...
  const close = async () => {
    ...
    setBusy("close");
    try {
      const result = await client.extension.invoke("cancelDuel", { duelId: local.duelId });
  ...
  return /* @__PURE__ */ jsxs("section", { className: "w-full min-w-full py-2 text-[12px]", "data-model-arena-duel": local.duelId, children: [
    /* @__PURE__ */ jsxs("div", { className: "mb-4 text-center", children: [
      /* @__PURE__ */ jsx("div", { className: "font-medium text-primary", children: "Model Arena duel" }),
      /* @__PURE__ */ jsx("div", { className: "mt-1 min-h-4 text-[11px] text-dim", children: busy === "close" ? "Closing..." : busy ? "Saving..." : null })
    ] }),
    /* @__PURE__ */ jsxs("div", { className: "grid w-full min-w-0 grid-cols-1 gap-5 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] md:gap-0", children: [
      /* @__PURE__ */ jsx(
    ...
    /* @__PURE__ */ jsxs("div", { className: "mt-4 flex flex-wrap justify-center gap-2", children: [
      /* @__PURE__ */ jsx(MessageActionButton, { className: "min-h-8 min-w-16 px-3 py-1.5", disabled: !ready || Boolean(busy), onClick: () => void vote("tie"), children: "Tie" }),
      /* @__PURE__ */ jsx(
        MessageActionButton,
        {
      ...
      /* @__PURE__ */ jsx(MessageActionButton, { className: "min-h-8 min-w-16 px-3 py-1.5", disabled: Boolean(busy), onClick: () => void close(), children: "Close" })
    ] }),
    visibleError ? /* @__PURE__ */ jsx("div", { className: "mt-2 text-center text-danger", children: visibleError }) : null
  ] });
}
function ModelArenaDuelAnswer({
  ...
  return /* @__PURE__ */ jsxs("article", { className: "flex min-w-0 flex-col md:border-r md:border-border-subtle md:pr-6 md:last:border-r-0 md:last:pl-6 md:last:pr-0", children: [
    /* @__PURE__ */ jsx("div", { className: "mb-2 text-[11px] font-medium uppercase text-dim", children: label }),
    /* @__PURE__ */ jsx("div", { className: "min-h-[10rem] min-w-0 flex-1 overflow-auto text-left text-[13px] leading-relaxed text-primary", children: waiting ? /* @__PURE__ */ jsx("div", { className: "text-dim", children: "Waiting for answer..." }) : failed ? /* @__PURE__ */ jsx("div", { className: "whitespace-pre-wrap break-words text-danger", children: text7 }) : renderMarkdownText(text7) }),
    /* @__PURE__ */ jsxs(MessageActionButton, { className: "mt-3 min-h-9 w-full justify-center px-3 py-2", disabled, onClick: onPrefer, children: [
      "Prefer ",
      label
    ] })
  ] });
...
var ContextShelf = memo(function ContextShelf2({
  ...
  const isWideArenaShelf = !hasSystemPrompt && !remoteControlled && blocks.length > 0 && blocks.every((block) => block.type === "context" && block.customType === "model_arena_duel");
  const shelfClassName = isWideArenaShelf ? "my-5 relative left-1/2 w-[min(96rem,calc(100vw_-_28rem))] min-w-full max-w-[calc(100vw_-_2rem)] -translate-x-1/2 space-y-1.5 text-dim" : "my-5 w-full max-w-[72rem] space-y-1.5 text-dim";
  ...
  return /* @__PURE__ */ jsxs("div", { className: shelfClassName, "data-context-shelf": "1", "data-context-shelf-layout": isWideArenaShelf ? "wide" : void 0, children: [
    ...
    blocks.map((block, index2) => {
      ...
      if (block.type === "context" && block.customType === "model_arena_duel") {
        return /* @__PURE__ */ jsx("div", { className: "my-4 w-full", children: /* @__PURE__ */ jsx(ModelArenaDuelContextBlock, { block }) }, modelArenaTranscriptBlockKey(block, index2));
      }
      if (block.type === "context" && block.customType) {
        ...
        const extension2 = renderer ? (extensionRegistry.extensions ?? []).find((candidate) => candidate.id === renderer.extensionId && candidate.enabled) : null;
        if (renderer && extension2) {
          return /* @__PURE__ */ jsx(
            "div",
            {
              ...
              children: /* @__PURE__ */ jsx(
                NativeExtensionTranscriptBlockHost,
                {
                  ...
                  renderer,
                  block,
                  context: { messageIndex: typeof messageIndexOffset === "number" ? messageIndexOffset + index2 : void 0 }
                }
              )
            },
            block.customType === "model_arena_duel" ? modelArenaTranscriptBlockKey(block, index2) : block.id ?? index2
          );
        }
      }
      return /* @__PURE__ */ jsx(
    ...
    "div",
    {
      ...
      children: /* @__PURE__ */ jsxs("div", { className: "min-w-0 flex-1 space-y-1.5", children: [
        /* @__PURE__ */ jsxs(MessageCard, { ...replySelectionScopeProps, className: "space-y-1 text-primary", children: [
          ...
          ) : renderStreamingPlainText ? renderStreamingMarkdownText(displayText, { onOpenFilePath, onOpenCheckpoint, validatedFilePathTargets }) : renderText(displayText, { onOpenFilePath, onOpenCheckpoint, validatedFilePathTargets }),
...
function AssistantVariationPager({
  ...
  const modelLabel = label ? ` \xB7 ${label}` : "";
  return /* @__PURE__ */ jsxs(
    "span",
    {
      ...
      children: [
        /* @__PURE__ */ jsxs("span", { className: "ui-tooltip-host relative inline-flex", children: [
          /* @__PURE__ */ jsx(
            MessageActionButton,
            {
              type: "button",
          ...
          /* @__PURE__ */ jsx(Tooltip, { position: "top-right", children: "Previous model response" })
        ] }),
        /* @__PURE__ */ jsxs("span", { className: "whitespace-nowrap tabular-nums", "aria-label": `Model response ${displayIndex} of ${total}${modelLabel}`, children: [
          "Version ",
          displayIndex,
          " of ",
          total
        ...
        /* @__PURE__ */ jsxs("span", { className: "ui-tooltip-host relative inline-flex", children: [
          ...
          /* @__PURE__ */ jsx(Tooltip, { position: "top-right", children: "Next model response" })
        ] })
      ]
    }
  );
...
function readFileChanges(details) {
  if (!isRecord8(details)) return [];
  const rawFileChanges = Array.isArray(details.fileChanges) ? details.fileChanges : isRecord8(details.result) && Array.isArray(details.result.fileChanges) ? details.result.fileChanges : [];
  return rawFileChanges.flatMap((candidate) => {
    if (!isRecord8(candidate)) return [];
    const path2 = readString3(candidate, "path");
    const status = readStatus(candidate.status);
    if (!path2 || !status) return [];
    return [
      {
        path: path2,
        previousPath: readString3(candidate, "previousPath"),
        status,
        additions: readNumber(candidate, "additions") ?? 0,
        deletions: readNumber(candidate, "deletions") ?? 0,
        patch: readString3(candidate, "patch"),
...
function readEditToolFileChanges(block) {
  if (block.tool !== "edit" || !isRecord8(block.input)) return [];
  const path2 = readString3(block.input, "path");
...
var TerminalToolBlock = memo(function TerminalToolBlock2({
  ...
  const completedWithoutOutput = !isRunning && !outputDeferred && !hasOutput && (block.status === "done" || block.status === "ok" || presentation.exitCode !== void 0 || presentation.cancelled);
...
${block.output}` : `$ ${presentation.command}`;
  ...
  return /* @__PURE__ */ jsxs("div", { className: "group space-y-1.5", onMouseEnter: prefetchDeferredOutput, onFocus: prefetchDeferredOutput, children: [
    /* @__PURE__ */ jsxs("div", { className: cx("ui-terminal-block", isError ? "text-danger" : null), children: [
      ...
      hasBody && /* @__PURE__ */ jsx("div", { className: "px-3 py-2.5 max-h-96 overflow-y-auto", children: hasOutput ? /* @__PURE__ */ jsx(
        "pre",
        {
          className: cx(
      ...
      ) : isRunning ? /* @__PURE__ */ jsx("p", { className: "ui-terminal-block__muted text-[11px] italic leading-relaxed", children: "Waiting for output\u2026" }) : outputDeferred ? /* @__PURE__ */ jsx("p", { className: "ui-terminal-block__muted text-[11px] italic leading-relaxed", children: "Older terminal output is available on demand." }) : completedWithoutOutput ? /* @__PURE__ */ jsx(
        "pre",
        {
...
function isRoutineActivityStatus(value) {
  return value === "passed" || value === "warned" || value === "blocked" || value === "failed" || value === "skipped";
}
function normalizeRoutineActivityStep(value) {
...
function normalizeRoutineActivityRun(value) {
  if (!value || typeof value !== "object") return null;
  const source = value;
  const id = typeof source.id === "string" ? source.id.trim() : "";
  const hookId = typeof source.hookId === "string" ? source.hookId.trim() : "";
  const position3 = source.position === "before" || source.position === "after" ? source.position : null;
  const steps = Array.isArray(source.steps) ? source.steps.map(normalizeRoutineActivityStep).filter((step) => Boolean(step)) : [];
  if (!id || !hookId || !position3 || !isRoutineActivityStatus(source.status) || steps.length === 0) return null;
  return { id, hookId, position: position3, status: source.status, steps };
}
function readRoutineActivityRuns(details) {
  if (!details || typeof details !== "object") return [];
  const root4 = details;
  const source = root4.result && typeof root4.result === "object" && !Array.isArray(root4.result) ? root4.result : root4;
  const routineHooks = source.routineHooks;
  return Array.isArray(routineHooks) ? routineHooks.map(normalizeRoutineActivityRun).filter((run) => Boolean(run)) : [];
}
...
function formatRoutineHookLabel(hookId) {
  if (hookId === "background.command") return "background command";
  if (hookId === "checkpoint") return "checkpoint";
  return hookId.replace(/[._-]+/g, " ").replace(/\s+/g, " ").trim();
}
function routineRunTitle(run) {
  const position3 = run.position === "before" ? "Before" : "After";
  const hook = formatRoutineHookLabel(run.hookId);
  return hook ? `${position3} ${hook}` : `${position3} hook`;
}
...
function routineStepSummary(step) {
...
function RoutineActivityRows({ runs }) {
  if (runs.length === 0) return null;
  return /* @__PURE__ */ jsx(Fragment2, { children: runs.map((run) => /* @__PURE__ */ jsxs("div", { className: "bg-black/5 px-2.5 py-2 font-sans text-[12px]", children: [
    /* @__PURE__ */ jsxs("div", { className: "mb-1.5 flex flex-wrap items-baseline gap-x-2 gap-y-1", children: [
      /* @__PURE__ */ jsx("span", { className: "text-[10px] uppercase tracking-[0.08em] text-dim", children: "Routine activity" }),
      /* @__PURE__ */ jsx("span", { className: "font-medium text-secondary", children: routineRunTitle(run) }),
      /* @__PURE__ */ jsx("span", { className: cx("text-[11px]", routineStatusClass(run.status)), children: run.status })
    ] }),
    /* @__PURE__ */ jsx("div", { className: "space-y-1", children: run.steps.map((step, index2) => /* @__PURE__ */ jsxs("div", { className: "grid grid-cols-[1.35rem_minmax(0,1fr)_auto] items-start gap-2", children: [
      /* @__PURE__ */ jsx("span", { className: "mt-0.5 w-4 shrink-0 text-right font-mono text-[10px] leading-4 tabular-nums text-secondary", children: index2 + 1 }),
      /* @__PURE__ */ jsxs("span", { className: "min-w-0", children: [
        /* @__PURE__ */ jsx("span", { className: "block truncate font-medium leading-4 text-primary", children: step.routineName }),
        step.message && step.outcome ? /* @__PURE__ */ jsx("span", { className: "block truncate text-[11px] leading-4 text-secondary", children: step.message }) : null
      ] }),
      /* @__PURE__ */ jsx("span", { className: cx("max-w-[11rem] truncate text-right text-[11px] leading-4", routineStatusClass(step.status)), children: routineStepSummary(step) })
    ] }, `${run.id}:${step.routineId}:${index2}`)) })
  ] }, run.id)) });
}
function ToolBlock({
  block,
  ...
  return /* @__PURE__ */ jsxs(
    "div",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ jsx(RoutineActivityRows, { runs: routineActivityRuns }),
        displayedLinkedRuns.length > 0 && !pinnedTool && !backgroundShellStart && /* @__PURE__ */ jsxs("div", { className: "border-t border-border-subtle/70 bg-black/5 px-2.5 py-2 text-[11px] font-sans", children: [
          ...
          hiddenRunCount > 0 && /* @__PURE__ */ jsxs("div", { className: "mb-2 flex flex-wrap items-center gap-2 rounded-md bg-black/5 px-2 py-1.5 text-[10px] text-secondary/80", children: [
            ...
            /* @__PURE__ */ jsx("span", { className: "flex-1" }),
            /* @__PURE__ */ jsx(Button, { variant: "action", onClick: toggleLinkedRuns, children: showAllRuns ? "Show fewer" : "Show all" })
          ] }),
  ...
  return /* @__PURE__ */ jsxs("div", { className: "space-y-1.5", children: [
        ...
        children: [
          /* @__PURE__ */ jsxs(
            RowButton,
            {
              ...
              children: [
                /* @__PURE__ */ jsxs("span", { className: "shrink-0 font-medium text-primary", children: [
                  summary.stepCount,
                  " step",
    ...
    open && /* @__PURE__ */ jsxs("div", { className: "ml-2.5 space-y-1.5 border-l border-border-subtle pl-2.5", children: [
      overflowBlockCount > 0 && /* @__PURE__ */ jsxs("div", { className: "flex flex-wrap items-center gap-2 rounded-md bg-elevated/35 px-2.5 py-1.5 text-[11px] text-secondary", children: [
        ...
        /* @__PURE__ */ jsx("span", { className: "flex-1" }),
        /* @__PURE__ */ jsx(Button, { variant: "action", onClick: toggleTraceClusterOverflow, children: showAllBlocks ? `Show latest ${MAX_VISIBLE_TRACE_BLOCKS}` : "Show all" })
      ] }),
...
var ErrorBlock = memo(function ErrorBlock2({
  ...
  return /* @__PURE__ */ jsx(Notice, { tone: "danger", className: "text-[12px] font-mono", children: /* @__PURE__ */ jsx("div", { className: "min-w-0 space-y-2", children: /* @__PURE__ */ jsxs("div", { ...replySelectionScopeProps, children: [
...
function ChatRenderItemView({
  ...
  if (item.type === "context_cluster") {
    ...
    return /* @__PURE__ */ jsxs("div", { "data-chat-tail": isTailContextItem ? "1" : void 0, style: contentVisibilityStyle, children: [
      item.blocks.map((_, offset) => {
        const absoluteIndex2 = messageIndexOffset + item.startIndex + offset;
        return /* @__PURE__ */ jsx("span", { id: `msg-${absoluteIndex2}`, className: "block h-0 overflow-hidden", "aria-hidden": true }, `anchor-${absoluteIndex2}`);
      }),
...
function readMessageBlockStableId(block) {
  const id = block.id;
  return typeof id === "string" && id.trim().length > 0 ? id.trim() : null;
}
function readModelArenaBlockStableId(block) {
...
function buildMessageRenderKey(block, absoluteIndex) {
...
function buildClusterRenderKey(item, messageIndexOffset) {
...
var ChatView = memo(function ChatView2({
  ...
  const baseStreamingStatusLabel = isCompacting ? "Compacting context\u2026" : pendingStatusLabel ?? getStreamingStatusLabel(messages, isStreaming);
  const streamingStatusLabel = baseStreamingStatusLabel && streamingThroughputLabel ? `${baseStreamingStatusLabel} \xB7 ${streamingThroughputLabel}` : baseStreamingStatusLabel;
