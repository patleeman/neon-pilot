...
} from "./chunk-4QWUSZJ3.js";
import "./chunk-4YPGCSK5.js";
import "./chunk-24DU7J3C.js";
import {
...
import "./chunk-WY4PXDOC.js";
import "./chunk-RUG6BXWL.js";
import {
  timeAgoCompact
...
var ConversationArtifactModal = lazy(
  () => import("./ConversationArtifactModal-RJWEDDZJ.js").then((module) => ({ default: module.ConversationArtifactModal }))
);
var ConversationDrawingsPickerModal = lazy(
  () => import("./ConversationDrawingsPickerModal-YS34RJFH.js").then((module) => ({ default: module.ConversationDrawingsPickerModal }))
);
var loadChatView = () => import("./ChatView-RQ4NOHYG.js").then((module) => ({ default: module.ChatView }));
var ChatView = lazy(loadChatView);
var ConversationActivityShelf = lazy(
  () => import("./ConversationActivityShelf-MOTXLLYB.js").then((module) => ({ default: module.ConversationActivityShelf }))
);
var ConversationContextShelf = lazy(
  () => import("./ConversationContextShelf-XHR6BVAN.js").then((module) => ({ default: module.ConversationContextShelf }))
);
var ConversationQuestionShelf = lazy(
  () => import("./ConversationQuestionShelf-2Y4OSGPD.js").then((module) => ({ default: module.ConversationQuestionShelf }))
);
var ConversationQueueShelf = lazy(
  () => import("./ConversationQueueShelf-VR35WKTL.js").then((module) => ({ default: module.ConversationQueueShelf }))
);
var ConversationSavedHeader = lazy(
  () => import("./ConversationSavedHeader-RC7KDTK6.js").then((module) => ({ default: module.ConversationSavedHeader }))
);
...
// packages/desktop/ui/src/components/workbench/WorkbenchBrowserTab.tsx
init_neon_pilot_shared_react();

// packages/desktop/ui/src/extensions/keybindings.ts
...
function matchesExtensionKeybinding(event, shortcut) {
...
function findMatchingExtensionKeybinding(event, keybindings, context = {}) {
  return keybindings.find(
    (keybinding) => keybinding.enabled !== false && evaluateCommandEnablement(keybinding.when, context) && keybinding.keys.some((shortcut) => matchesExtensionKeybinding(event, shortcut))
  ) ?? null;
}

// packages/desktop/ui/src/local/workbenchBrowserTabs.ts
var STORAGE_KEY = "pa:workbench-browser-tabs";
var DEFAULT_URL = "https://www.google.com/";
var BROWSER_TABS_CHANGED_EVENT = "pa:system-browser-tabs-changed";
var STORAGE_GUARD_FLAG = "__neonPilotWorkbenchBrowserTabsStorageGuard";
function generateId() {
...
function validateState(raw) {
  ...
  const hasActive = state.tabs.some((t) => {
  ...
  const closedTabs = Array.isArray(state.closedTabs) ? state.closedTabs.map((t) => normalizeTab(t)).filter((t) => t !== null).slice(0, 10) : [];
  return {
    version: 1,
    tabs: state.tabs.map((t) => normalizeTab(t)).filter((t) => t !== null),
...
function normalizeRawState(raw) {
  ...
  const state = raw;
  const tabs = Array.isArray(state.tabs) ? state.tabs.map((t) => normalizeTab(t)).filter((t) => t !== null) : [];
...
function reconcileBrowserTabsStateForWrite(next, current) {
  ...
  const activeTabId = current.activeTabId && !nextTabIds.has(current.activeTabId) && !nextClosedTabIds.has(current.activeTabId) ? current.activeTabId : normalized.activeTabId;
  return {
    state: {
      ...normalized,
      tabs,
...
function readStoredBrowserTabsState(storage) {
  try {
...
function dispatchReconciledBrowserTabsState(state) {
  if (typeof window === "undefined") return;
  queueMicrotask(() => {
    window.dispatchEvent(new CustomEvent(BROWSER_TABS_CHANGED_EVENT, { detail: state }));
  });
}
function installBrowserTabsStorageGuard() {
  ...
  Storage.prototype.setItem = function guardedSetItem(key, value) {
    ...
    try {
      const next = normalizeRawState(JSON.parse(String(value)));
      if (!next) {
        return originalSetItem.call(this, key, value);
      }
...
function writeBrowserTabsState(state) {
  try {
...
function getAdjacentTabId(state, closedTabId) {
...
// packages/desktop/ui/src/components/workbench/WorkbenchBrowserTab.tsx
var WORKBENCH_BROWSER_COMMENT_ADDED_EVENT = "pa:workbench-browser-comment-added";
var WORKBENCH_BROWSER_COMMAND_EVENT = "neon-pilot-workbench-browser-command";
var WORKBENCH_BROWSER_SHORTCUT_COMMANDS = /* @__PURE__ */ new Set(["browser.newTab", "browser.reopenTab", "browser.closeTab", "browser.focusLocation"]);
function hasBlockingHtmlModal() {
  if (typeof document === "undefined") {
    return false;
  }
  return Boolean(document.querySelector('[aria-modal="true"]'));
}
function formatWorkbenchBrowserError(error) {
  ...
  if (!message || raw.includes("\n") || /Local API route did not complete/i.test(raw) || /\/api\//i.test(raw) || /file:\/\//i.test(raw) || /localApi\.js/i.test(raw) || /\bModule\.[A-Za-z_$][\w$]*/.test(raw) || /packages\/desktop\//i.test(raw) || /\s+at\s+\S+/i.test(raw)) {
...
function WorkbenchBrowserTab({
  ...
  const [surfaceKeybindings, setSurfaceKeybindings] = useState([]);
  ...
  const tabUrlMapRef = useRef({});
  useEffect(() => {
    const draft = activeTab.urlDraft || activeTab.url;
    urlDraftRef.current = draft;
    setUrlDraft(draft);
  }, [activeTab.id]);
  const [navigatedTabs, setNavigatedTabs] = useState(() => /* @__PURE__ */ new Set());
  useEffect(() => {
    if (!bridge || !activeTab.url || navigatedTabs.has(activeTab.id)) {
      return;
    }
    setNavigatedTabs((prev) => new Set(prev).add(activeTab.id));
    void bridge.navigateWorkbenchBrowser({ url: activeTab.url, sessionKey: browserSessionKey }).then((nextState) => {
      if (nextState) {
  ...
  const syncUrlDraftFromBrowserState = useCallback((nextState, tabId) => {
  ...
  const syncBounds = useCallback(() => {
    ...
    if (hasBlockingHtmlModal()) {
      void bridge.setWorkbenchBrowserBounds({ visible: false, sessionKey: browserSessionKey }).then((nextState) => {
        if (nextState) {
          if (tabsStateRef.current.activeTabId === activeTab.id) {
            setState(nextState);
          }
  ...
  useEffect(() => {
    ...
    void bridge.setWorkbenchBrowserBounds({ visible: false, sessionKey: previousSessionKey, deactivate: true }).catch(() => void 0);
  ...
  useLayoutEffect(() => {
    ...
    const timer = window.setInterval(syncBounds, 1e3);
    return () => {
      ...
      for (const tab of currentTabs) {
        void bridge?.setWorkbenchBrowserBounds({ visible: false, sessionKey: getTabSessionKey(tab.id), deactivate: true }).catch(() => void 0);
      }
    };
  ...
  useEffect(() => {
    let cancelled = false;
    const load = () => {
      api.extensionKeybindings().then((keybindings) => {
        if (!cancelled) {
          setSurfaceKeybindings(
            keybindings.filter(
              (keybinding) => keybinding.enabled && keybinding.scope === "surface" && WORKBENCH_BROWSER_SHORTCUT_COMMANDS.has(keybinding.command)
            )
          );
        }
      }).catch(() => {
  ...
  useEffect(() => {
    function handleBrowserCommand(event) {
      ...
      if (command === "reloadOrStop") {
        void runBrowserCommand(
          () => state?.loading ? bridge.stopWorkbenchBrowser({ sessionKey: browserSessionKey }) : bridge.reloadWorkbenchBrowser({ sessionKey: browserSessionKey })
        );
      }
    }
  ...
  useEffect(() => {
    function handleBrowserCommentTarget(event) {
    ...
    try {
      ...
      if (nextState) {
        ...
        onSetTabsState((prev) => ({
          ...prev,
          tabs: prev.tabs.map(
            (t) => t.id === commandTabId ? { ...t, url: nextState.url, urlDraft: newUrl, title: nextState.title || t.title } : t
          )
        }));
      }
      setStatus("");
  ...
  return /* @__PURE__ */ jsxs("div", { className: "flex h-full min-h-0 flex-col overflow-hidden", children: [
        ...
        children: [
          ...
          /* @__PURE__ */ jsx(
            IconButton,
            {
              ...
              onClick: () => void runBrowserCommand(
                () => state?.loading ? bridge.stopWorkbenchBrowser({ sessionKey: browserSessionKey }) : bridge.reloadWorkbenchBrowser({ sessionKey: browserSessionKey })
              ),
              children: state?.loading ? "\xD7" : "\u21BB"
            }
          ),
          /* @__PURE__ */ jsx(
            TextInput,
          ...
          /* @__PURE__ */ jsx(IconButton, { compact: true, size: "sm", "aria-label": "Close browser tab", title: "Close browser tab", onClick: handleCloseCurrentBrowserTab, children: "\xD7" })
        ]
      }
    ),
    /* @__PURE__ */ jsxs("div", { ref: browserHostRef, className: "relative min-h-[220px] flex-1 overflow-hidden bg-base", children: [
      !bridge ? /* @__PURE__ */ jsx("div", { className: "flex h-full items-center justify-center px-4 text-center text-[12px] leading-5 text-dim", children: "Browser embedding is only available in the Electron desktop app." }) : null,
      pendingMarkers.map((marker, index) => {
        ...
        return /* @__PURE__ */ jsx("div", { className: "ui-workbench-drop-badge", style: { left: x, top: y }, title: marker.comment, "aria-hidden": "true", children: index + 1 }, marker.id);
      }),
      commentDraft ? /* @__PURE__ */ jsxs(
        "div",
        {
          ...
          children: [
            /* @__PURE__ */ jsxs("p", { className: "truncate text-[11px] font-medium text-primary", children: [
              "Comment on ",
              commentDraft.target.role ?? "element",
            ...
            /* @__PURE__ */ jsxs("div", { className: "mt-2 flex justify-end gap-1.5", children: [
              /* @__PURE__ */ jsx(ToolbarButton, { onClick: () => setCommentDraft(null), children: "Cancel" }),
              /* @__PURE__ */ jsxs(Button, { variant: "action", onClick: saveCommentDraft, children: [
                /* @__PURE__ */ jsx("span", { "aria-hidden": "true", children: "+" }),
                "Add comment"
              ] })
            ] })
    ...
    status ? /* @__PURE__ */ jsx("div", { className: "shrink-0 border-t border-border-subtle px-3 py-1.5 text-[11px] text-dim", children: status }) : null
  ] });
}

// extensions/system-browser/src/panels.tsx
init_neon_pilot_shared_react();

// extensions/system-browser/src/BrowserToolBlock.tsx
init_neon_pilot_shared_react();
var BROWSER_TOOL_LABELS = {
  browser_snapshot: "Browser snapshot",
  browser_cdp: "Browser action",
  browser_screenshot: "Browser screenshot"
};
function readBrowserUrl(block) {
  const details = block.details && typeof block.details === "object" ? block.details : null;
...
var BrowserToolBlock = memo(function BrowserToolBlock2({
  block,
  onOpenBrowser
}) {
  ...
  return /* @__PURE__ */ jsx(
    ToolResultCard,
    {
      ...
      title,
      badges: /* @__PURE__ */ jsx(Pill, { tone: isError ? "danger" : "teal", mono: true, children: "browser" }),
      meta: url ? /* @__PURE__ */ jsx("span", { className: "font-mono text-secondary", children: url }) : void 0,
      body: isError && block.output ? block.output : "The agent used the Workbench Browser. Open it when you want to see or control the page.",
      actions: isRunning ? /* @__PURE__ */ jsxs(InlineMeta, { children: [
        /* @__PURE__ */ jsx(Spinner, {}),
        "using browser\u2026"
      ] }) : /* @__PURE__ */ jsx(TextButton, { type: "button", onClick: () => onOpenBrowser?.(), disabled: !onOpenBrowser, tone: "accent", children: "Open browser" })
    }
  );
});

// extensions/system-browser/src/panels.tsx
var BROWSER_TABS_CHANGED_EVENT2 = "pa:system-browser-tabs-changed";
var WORKBENCH_OPEN_TOOL_TAB_EVENT = "pa:workbench-open-tool-tab";
function BrowserTranscriptRenderer({ block, context }) {
  return /* @__PURE__ */ jsx(BrowserToolBlock, { block, onOpenBrowser: context.onOpenBrowser });
}
var browserTabsSnapshot = readBrowserTabsState();
function publishBrowserTabsState(next) {
  browserTabsSnapshot = next;
  writeBrowserTabsState(next);
  window.dispatchEvent(new CustomEvent(BROWSER_TABS_CHANGED_EVENT2, { detail: next }));
}
function useBrowserTabsState() {
  const [tabsState, setTabsState] = useState(browserTabsSnapshot);
  useEffect(() => {
    const handleChange = (event) => {
      setTabsState(event.detail ?? browserTabsSnapshot);
    };
    window.addEventListener(BROWSER_TABS_CHANGED_EVENT2, handleChange);
    return () => window.removeEventListener(BROWSER_TABS_CHANGED_EVENT2, handleChange);
  }, []);
  const updateTabsState = useCallback((updater) => {
...
function useBrowserTabActions() {
  ...
  const closeTab = useCallback(
    (tabId) => {
      ...
      void getDesktopBridge()?.setWorkbenchBrowserBounds({ visible: false, sessionKey: getTabSessionKey(tabId), deactivate: true }).catch(() => void 0);
    },
    [setTabsState]
  );
...
function BrowserTabsPanel() {
  ...
  return /* @__PURE__ */ jsxs(RailSection, { title: "Browser", bodyClassName: "flex flex-col gap-px px-1.5 py-1.5", children: [
    tabsState.tabs.map((tab) => /* @__PURE__ */ jsx(
      ResourceListItem,
      {
        label: tab.title,
    ...
    /* @__PURE__ */ jsx(ResourceListItem, { label: "New tab", leading: "+", onClick: addTab, title: "New tab", "aria-label": "New tab" })
  ] });
}
function BrowserWorkbenchPanel({ context }) {
  const { tabsState, setTabsState, addTab, closeTab, reopenTab } = useBrowserTabActions();
  const workbenchTabId = context?.instanceId ?? null;
  const activeTab = workbenchTabId ? tabsState.tabs.find((tab) => tab.id === workbenchTabId) ?? {
    id: workbenchTabId,
    title: "New Tab",
