...
function evaluateCommandEnablement(expression, context = {}) {
  const trimmed = expression?.trim();
  if (!trimmed) return true;
  const negated = trimmed.startsWith("!");
  const body = negated ? trimmed.slice(1).trim() : trimmed;
  const comparison = body.match(/^([A-Za-z0-9_.:-]+)\s*([!=]=)\s*(.+)$/);
  const result = comparison ? compareContextValue(readContextValue(context, comparison[1]), comparison[2], comparison[3]) : Boolean(readContextValue(context, body));
  return negated ? !result : result;
}
function compareContextValue(value, operator, expected) {
