...
The built-in Browser is the user-facing browser surface inside the desktop Workbench layout. In the Tauri shell it is backed by Rust-owned child webviews positioned under the React Browser tab.
...
- browser snapshot/action behavior in the native desktop browser bridge
- the boundary between the built-in browser and the `agent-browser` CLI/dev tool

## Product model

The Workbench Browser is a desktop-owned embedded webview. The React tab reports bounds through the Tauri bridge; Rust owns the child webviews, navigation, visibility, tab state, and the sidecar bridge used by agent browser tools.
...
- **Workbench Browser**: product UI surface in the desktop shell; user-visible; shared communication context between user and agent; supports comments and built-in browser tools.
...
Desktop browser tools should use the Workbench Browser session directly instead of launching an unrelated `agent-browser` session. The Tauri bridge supports active-state checks, tab listing, navigation state, text snapshots, and screenshot capture. Raw Chromium CDP commands are not available in the Tauri WKWebView backend.
...
- `packages/desktop/ui/src/components/workbench/WorkbenchBrowserTab.tsx` — Browser tab UI and Tauri webview bounds synchronization
- `packages/tauri/desktop-shell/src-tauri/src/main.rs` — Rust child webview ownership and sidecar browser bridge
