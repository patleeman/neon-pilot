...
Use `/code`, `/code on`, `/code off`, or `/code status` in the composer to change the active conversation. When Code Mode is on, the composer shelf shows a compact status notice above the input area with a Turn off action.
