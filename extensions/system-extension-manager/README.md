...
Users can add more GitHub extension repositories from the install dialog. Repositories are stored in `settings.json` as `extensions.sources`, fetched from each repo's root `neon.extensions.json`, and merged into the installable extension catalog. The first-party Neon Pilot extension repo remains the built-in default source.
