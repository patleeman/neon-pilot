...
import { ToolbarButton } from '@personal-agent/extensions/ui';
...
function toneClass(active: boolean) {
  return active
    ? 'border-accent/80 bg-accent/10 shadow-[0_0_0_1px_rgba(var(--color-accent),0.25),0_18px_55px_rgba(0,0,0,0.24)]'
...
export function LocalModelsPage({ pa }: ExtensionSurfaceProps) {
  ...
  return (
    <div className="h-full overflow-y-auto bg-[radial-gradient(circle_at_50%_0%,rgba(var(--color-accent),0.08),transparent_34rem)]">
      <div className="mx-auto flex min-h-full w-full max-w-[88rem] flex-col gap-5 px-5 py-6">
        <header className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h1 className="text-[28px] font-semibold leading-tight tracking-[-0.04em] text-primary">Local Models</h1>
            <p className="mt-1 text-sm text-secondary">A local model studio for MLX and GGUF runtimes.</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <div className="inline-flex items-center gap-2 rounded-lg border border-border-subtle bg-surface/55 px-3 py-2 text-sm text-secondary">
        ...
        <div className="grid min-h-0 flex-1 gap-4 xl:grid-cols-[20rem_minmax(0,1fr)_18rem]">
          <aside className="space-y-4 rounded-xl border border-border-subtle bg-surface/45 p-4 shadow-[0_18px_55px_rgba(0,0,0,0.18)]">
            ...
            <div className="space-y-2">
              ...
              {library.map((model) => (
                <button
                  ...
                  className={`w-full rounded-lg border p-3 text-left transition-colors ${toneClass(model.id === selected?.id)}`}
                >
            ...
            <div className="space-y-2 border-t border-border-subtle pt-4">
              ...
              {(searchResults.length ? searchResults : []).slice(0, 4).map((model) => (
                <button
                  key={model.id}
                  type="button"
                  className="block w-full rounded-lg border border-border-subtle bg-surface/35 p-3 text-left hover:border-accent/50"
                  onClick={() => {
              ...
              <button
                type="button"
                className="w-full rounded-lg border border-border-subtle px-3 py-2 text-sm text-secondary hover:text-primary"
                onClick={() => void searchMlx()}
              >
          ...
          <main className="min-w-0 space-y-4">
            <section className="rounded-xl border border-accent/60 bg-surface/50 p-5 shadow-[0_18px_70px_rgba(var(--color-accent),0.14)]">
            ...
            <section className="rounded-xl border border-border-subtle bg-surface/45 p-5">
              ...
              <div className="mt-4 rounded-lg border border-border-subtle bg-surface/40 p-3">
                ...
                <textarea
                  ...
                  className="min-h-24 w-full resize-y bg-transparent text-sm text-primary outline-none"
                />
              </div>
              <div className="mt-4 rounded-lg border border-border-subtle bg-[#0f131c]/80 p-4">
          ...
          <aside className="space-y-4">
            <section className="rounded-xl border border-border-subtle bg-surface/45 p-4">
        ...
        {logsOpen ? (
          <pre className="max-h-56 overflow-auto rounded-xl border border-border-subtle bg-[#0f131c] p-4 text-xs text-secondary">
